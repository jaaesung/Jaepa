...
class NewsSourcesHandler:
        ...
        try:
            ...
            for news in general_news:
                if (keyword_lower in news.get('headline', '').lower() or
                    ...
                    keyword_lower in news.get('category', '').lower()):
            ...
            try:
                from crawling.mock_data import get_mock_finnhub_news
                mock_news = get_mock_finnhub_news(keyword)
                logger.info(f"Finnhub API 호출 실패, 모의 데이터 사용 ({len(mock_news)}개)")
                ...
                for article in mock_news:
                    try:
                        normalized = {
                            ...
                            "url": article.get("url", ""),
                            "published_date": self._format_finnhub_date(article.get("datetime", 0)),
                            "source": article.get("source", "Finnhub (Mock)"),
                            "source_type": "finnhub_mock",
                            "crawled_date": datetime.now().isoformat(),
                        ...
                        logger.error(f"Finnhub 모의 뉴스 정규화 중 오류: {str(e)}")
            except Exception as e:
                logger.error(f"모의 데이터 로드 중 오류: {str(e)}")
    ...
    def _search_newsdata_news(self, keyword: str, days: int = 7) -> List[Dict[str, Any]]:
        ...
        while page_count < max_pages:
            ...
            if status_code != 200 or not response_text:
                ...
                # API 호출 실패 시 모의 데이터 사용
                try:
                    from crawling.mock_data import get_mock_newsdata_response
                    mock_response = get_mock_newsdata_response(keyword)
                    logger.info(f"NewsData.io API 호출 실패, 모의 데이터 사용 ({len(mock_response['results'])}개)")
                    response_text = json.dumps(mock_response)
                    status_code = 200  # 모의 데이터를 사용하도록 상태 코드 변경
                except Exception as e:
                    logger.error(f"NewsData.io 모의 데이터 로드 중 오류: {str(e)}")
                    break

            if not response_text:  # 모의 데이터도 없는 경우
                break
    ...
    def search_news(self, keyword: str, days: int = 7) -> List[Dict[str, Any]]:
        """
        여러 API를 통해 뉴스 검색
        ...
        if self.finnhub_client:
            try:
                finnhub_articles = self._search_finnhub_news(keyword, days)
                if finnhub_articles:
                    all_articles.extend(finnhub_articles)
                    logger.info(f"Finnhub API에서 {len(finnhub_articles)}개의 기사를 가져왔습니다.")
            except Exception as e:
                logger.error(f"Finnhub API 검색 오류: {str(e)}")

        # NewsData.io API를 통한 검색
        if self.newsdata_api_key:
            try:
                newsdata_articles = self._search_newsdata_news(keyword, days)
                if newsdata_articles:
                    all_articles.extend(newsdata_articles)
                    logger.info(f"NewsData.io API에서 {len(newsdata_articles)}개의 기사를 가져왔습니다.")
            except Exception as e:
                logger.error(f"NewsData.io API 검색 오류: {str(e)}")

        # 감성 분석 추가
        if hasattr(self, 'sentiment_analyzer') and self.sentiment_analyzer and all_articles:
        ...
        # MongoDB에 저장
        if all_articles:
            self._save_to_mongodb(all_articles)
    ...
    def _save_to_mongodb(self, articles: List[Dict[str, Any]]) -> None:
        ...
        """
        if not hasattr(self, 'news_collection') or self.news_collection is None:
