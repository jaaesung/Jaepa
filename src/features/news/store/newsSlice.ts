...
export const fetchNewsItem = createAsyncThunk<
  ...
  string,
  { rejectValue: string }
>("news/fetchNewsItem", async (newsId, { rejectWithValue }) => {
  try {
...
export const fetchNewsSources = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("news/fetchSources", async (_, { rejectWithValue }) => {
  try {
...
const newsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchNews.fulfilled,
        (state, action: PayloadAction<PaginatedNewsResponse>) => {
      ...
      .addCase(
        fetchNewsKeywords.fulfilled,
        (state, action: PayloadAction<KeywordData[]>) => {
...
export const { clearNewsError, setSelectedArticle, clearSelectedArticle } =
  newsSlice.actions;
