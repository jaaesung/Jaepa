...
const NewsCard: React.FC<NewsCardProps> = ({ article, onClick }) => {
  ...
  return (
    <Card className="news-card" onClick={handleClick}>
      <div className="news-card-header">
        <h3 className="news-card-title">{article.title}</h3>
        {article.sentiment && article.sentiment.label && (
          <div className={`news-card-sentiment ${getSentimentClass()}`}>
            {getSentimentText()}
          </div>
        )}
      </div>
      ...
      <div className="news-card-footer">
        ...
        <div className="news-card-date">{formatDate(article.publishedDate)}</div>
      </div>
