...
>(
  ...
  async ({ symbols, period = '1m' }, { rejectWithValue }) => {
    try {
...
export const fetchStockCorrelation = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  'stock/fetchCorrelation',
  async ({ symbol, startDate, endDate }, { rejectWithValue }) => {
    try {
      return await apiClient.stocks.getCorrelation(symbol, { startDate, endDate });
    } catch (error: any) {
    ...
    builder
      ...
      .addCase(fetchMultipleStocks.fulfilled, (state, action: PayloadAction<Record<string, StockData>>) => {
        ...
        Object.values(action.payload).forEach(stockData => {
          const existingIndex = state.stocks.findIndex(stock => stock.symbol === stockData.symbol);
          if (existingIndex >= 0) {
      ...
      .addCase(fetchStockCorrelation.fulfilled, (state, action: PayloadAction<CorrelationData[]>) => {
      ...
      .addCase(searchStocks.fulfilled, (state, action: PayloadAction<Array<{ symbol: string; name: string }>>) => {
...
export const { clearStockError, setSelectedStock, clearSelectedStock, clearSearchResults } = stockSlice.actions;
