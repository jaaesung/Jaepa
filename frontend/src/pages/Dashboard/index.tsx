...
const Dashboard: React.FC = () => {
  ...
  const { articles: newsItems, isLoading: newsLoading } = useSelector((state: RootState) => state.news);
  const { stocks: stockData, isLoading: stockLoading } = useSelector((state: RootState) => state.stocks);
  
  const [selectedPeriod, setSelectedPeriod] = useState<PeriodType>('1w');
  const [selectedStocks, setSelectedStocks] = useState<string[]>(['AAPL', 'MSFT', 'GOOG']);
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        <h1>대시보드</h1>
        <p className="welcome-message">안녕하세요, {user?.username || '사용자'}님!</p>
      </div>
