...
def handle_news_command(args: argparse.Namespace) -> None:
    ...
    if args.news_command == 'search':
        ...
        # API 소스 추가 (Finnhub, NewsData.io)
        if sources is None:
            sources = list(crawler.rss_feeds.keys()) + ['finnhub', 'newsdata']
        ...
        if articles:
            ...
            for i, article in enumerate(articles[:15], 1):  # 최대 15개만 표시
                print(f"{i}. {article['title']}")
                print(f"   출처: {article['source']} | 날짜: {article.get('published_date', '')[:10]}")
                ...
                if 'keywords' in article and article['keywords']:
                    print(f"   키워드: {', '.join(article['keywords'][:10])}")
                print()
        else:
            print(f"키워드 '{args.keyword}'에 대한 뉴스를 찾지 못했습니다.")
            print("\n다음 방법을 시도해 보세요:")
            print("1. 다른 키워드 사용 (예: 'bitcoin' 대신 'crypto')")
            print("2. 검색 기간 확장 (--days 옵션 사용)")
            print("3. 특정 소스 지정 (--sources finnhub newsdata)")
